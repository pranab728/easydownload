...
<div class="card">
	<div class="d-sm-flex align-items-center justify-content-between">
		<h5 class=" mb-0 text-gray-800 pl-3">{{ __('Resubmit Pending Products') }}</h5>
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
			<li class="breadcrumb-item"><a href="javascript:;">{{ __('Products') }}</a></li>
			<li class="breadcrumb-item"><a href="{{ route('admin.product.completed') }}">{{ __('Resubmit Pending Product') }}</a></li>
		</ol>
	</div>
</div>
...
<div class="modal fade" id="commentModal" data-itemid="" tabindex="-1" role="dialog" aria-labelledby="commentModal" aria-hidden="true">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="commentForm" action="{{ route('admin.item.soft') }}" method="post">
                        {{ csrf_field() }}
                        <input type="hidden" name="itemid" value="">
                        <div class="form-group">
                            <label for="">{{__('Comment')}} *</label>
                            <textarea name="temp_comment" class="form-control" rows="5" cols="80" required></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               searching: false,
               ajax: '{{ route('admin.product.resubmit.datatables','1') }}',
               columns: [

                        { data: 'temp_thumbnail_image', name: 'temp_thumbnail_image' , searchable: false, orderable: false},
                        { data: 'name', name: 'name' },
                        { data: 'category_id', name: 'category_id' },
                        { data: 'regular_price', name: 'regular_price' },
                        { data: 'demo_link', name: 'demo_link' },
                        { data: 'temp_mainfile', name: 'temp_mainfile' },
                        { data: 'user_id', name: 'user_id' },
                        { data: 'sell', name: 'sell' },
                        { data: 'temp_status', name: 'temp_status' },
