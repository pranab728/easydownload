...
@section('content')
    ...
    <div class="card">
        ...
        <h5 class=" mb-0 text-gray-800 pl-3">{{ __('Statement') }}</h5>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
            <li class="breadcrumb-item"><a href="javascript:;">{{ __('Manage Orders') }}</a></li>
            <li class="breadcrumb-item"><a href="{{ route('admin.order.index') }}">{{ __('All Statements') }}</a></li>
        </ol>
        </div>
    </div>
...
var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               searching: false,
               ajax: '{{ route('admin.statement.datatables') }}',
               columns: [
                        { data: 'ordernumber', searchable: true, orderable: false },
                        { data: 'invoice', searchable: true, orderable: false },
                        { data: 'date', searchable: true, orderable: false },
                        { data: 'details', searchable: true, orderable: false  },
                        { data: 'price', name: 'price' },
                        { data: 'adminprofit', name: 'adminprofit' }
