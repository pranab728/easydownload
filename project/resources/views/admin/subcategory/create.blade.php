...
<div class="card">
    <div class="d-sm-flex align-items-center justify-content-between">
      <h5 class=" mb-0 text-gray-800 pl-3">{{ __('Add New SubCategory') }} <a class="btn btn-primary btn-rounded btn-sm" href="{{route('admin.subcat.index')}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h5>
      <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="{{route('admin.subcat.create')}}">{{ __('Add New SubCategory') }}</a></li>
      </ol>
    </div>
</div>
  ...
  <div class="col-lg-6">
    ...
    <div class="card mb-4">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-primary">{{ __('Add New SubCategory Form') }}</h6>
      </div>

      <div class="card-body">
        <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
        <form class="geniusform" action="{{route('admin.subcat.store')}}" method="POST" enctype="multipart/form-data">
