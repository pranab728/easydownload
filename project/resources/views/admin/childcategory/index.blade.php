...
@section('content')
    ...
    <div class="card">
        ...
        <h5 class=" mb-0 text-gray-800 pl-3">{{ __('ChildCategories') }}</h5>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
            <li class="breadcrumb-item"><a href="javascript:;">{{ __('Manage Categories') }}</a></li>
            <li class="breadcrumb-item"><a href="{{ route('admin.childcat.index') }}">{{ __('ChildCategories') }}</a></li>
        </ol>
        </div>
    </div>
...
<div class="modal fade confirm-modal" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">{{__("You are about to delete this ChildCategory. Every informtation under this childcategory will be deleted.")}}</p>
                <p class="text-center">{{ __("Do you want to proceed?") }}</p>
            </div>
            <div class="modal-footer">
                <a href="javascript:;" class="btn btn-secondary" data-dismiss="modal">{{ __("Cancel") }}</a>
...
var table = $('#geniustable').DataTable({
        ...
        $(function() {
            $(".btn-area").append('<div class="col-sm-12 col-md-4 pr-3 text-right">'+
                '<a class="btn btn-primary" href="{{route('admin.childcat.create')}}">'+
            '<i class="fas fa-plus"></i> {{__('Add New ChildCategory')}}'+
            '</a>'+
            '</div>');
        });
