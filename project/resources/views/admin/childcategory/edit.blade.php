...
<div class="card">
    ...
    <h5 class=" mb-0 text-gray-800 pl-3">{{ __('Edit ChildCategory') }} <a class="btn btn-primary btn-rounded btn-sm" href="{{route('admin.childcat.index')}}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a></h5>
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
        <li class="breadcrumb-item"><a href="javascript:;">{{ __('Manage Categories') }}</a></li>
        <li class="breadcrumb-item"><a href="{{ route('admin.childcat.index') }}">{{ __('Main ChildCategoryCategories') }}</a></li>
        <li class="breadcrumb-item"><a href="{{route('admin.subcat.edit',$data->id)}}">{{ __('Edit ChildCategory') }}</a></li>
    </ol>
    </div>
</div>
  ...
  <div class="col-lg-6">
    ...
    <div class="card mb-4">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-primary">{{ __('Edit ChildCategory Form') }}</h6>
      </div>

      <div class="card-body">
        <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
        <form class="geniusform" action="{{route('admin.childcat.update',$data->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <select id="subcat"  name="subcategory_id" required="" class="form-control">
