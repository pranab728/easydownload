...
@section('content')
			...
			<section class="gray-light">
				<div class="container">
          <form id="geniusformdata" action="{{route('user.product.update',$data->id)}}" enctype="multipart/form-data" method="POST">
              {{csrf_field()}}
              ...
              <div class="lit98_jhy">
                ...
                <div class="form-group">
                    <label for="inp-name">{{ __('Sub Category') }}</label>
                    <select id="subcat" class="form-control mb-3" name="subcategory_id">
                      <option value="{{ $data->category->sub }}">{{ __('Select Sub Category') }}</option>
                      ...
                      @else
                            @foreach ($subcategories as $sub)
                            <option data-href="{{ route('user.subcat.load',$sub->id) }}" value="{{ $sub->id }}" {{$sub->id == $data->subcategory_id ? "selected":""}}>{{$sub->name}}</option>
                            @endforeach
                ...
                <div class="form-group">
                  ...
                  <select id="childcat" class="form-control mb-3" name="childcategory_id" >
                      <option value="{{ $data->category->sub }}">{{ __('Select Child Category') }}</option>
                      @if($data->subcategory_id != null)
      ...
      <br>
        ...
        <div class="lit98_jhy">
            ...
            <div class="form-group">
                ...
                <div class="wrapper-image-preview">
                    <div class="box">
                        <div class="back-preview-image" style="background-image: url({{$data->thumbnail_imagename ? asset('assets/images/'.$data->thumbnail_imagename) : asset('assets/images/placeholder.jpg') }});"></div>
            ...
            <div class="form-group">
                ...
                <div class="wrapper-image-preview">
                    <div class="box">
                        <div class="back-preview-image" style="background-image: url({{$data->preview_imagename ? asset('assets/images/'.$data->preview_imagename) : asset('assets/images/placeholder.jpg') }});"></div>
                        <div class="upload-options">
            ...
            <div>
                @foreach($data->screenshots as $screensort)
                   ...
                   <p class="m-2 p-1"><a href="{{route('user.screensort.remove',$screensort->id)}}" class="btn btn-danger btn-sm"><i class="fa fa-trash" aria-hidden="true"></i></a></p>
                @endforeach
            </div>
        </div>
        <br>
        ...
        <div class="lit98_jhy">
            ...
            @endif
						...
						<div class="col-lg-4 col-md-12 col-sm-12">
                            ...
                            <div class="lit98_jhy mb-2">
                                <div class="form-check form-check-lg">
                                    <input class="form-check-input" type="checkbox" id="acc_status" name="access_status" value="{{ $data->access_status }}" {{ $data->access_status==1 ? 'checked':'' }}>
              ...
              <div class="lit98_jhy price-section {{ $data->access_status==1 ? 'd-none':'' }} ">
                  <div class="extra_ijyu98_head">
                  ...
                  <div class="form-group">
                    ...
                    <input type="number" id="regularPrice" class="form-control" name="regular_price" placeholder="{{ __('Regular Price') }}" value="{{$data->access_status==1 ? 0 :round(($data->regular_price*$curr->value)-$category->regular_buyer_fee,2)}}" step="0.01" min="0">
                    <p class="text-center mb-0">+</p>
                    <p class="text-center"><span>{{$curr->sign}} {{ round($category->regular_buyer_fee * $curr->value,2)}} {{__('(Buyer Fee)')}}</span> = <span>{{$curr->sign}} <span id="totalRegularPrice">{{ round(($data->regular_price*$curr->value),2)}}</span> ({{__('Purchase Price')}})</span></p>
                </div>
                <div class="form-group">
                    <label for="extended_price">{{ __('Extended Price *') }}</label>
                    <input type="number" id="extendedPrice" class="form-control" name="extended_price" placeholder="{{ __('Extended Price *') }}" value="{{$data->access_status==1 ? 0 :round(($data->extended_price*$curr->value)-$category->extended_buyer_fee,2)}}" step="0.01" min="0">
                    <p class="text-center mb-0">+</p>
                    <p class="text-center"><span>{{$curr->sign}} {{ round($category->extended_buyer_fee * $curr->value,2)}} {{__('(Buyer Fee)')}}</span> = <span>{{$curr->sign}} <span id="totalExtendedPrice">{{ round(($data->extended_price*$curr->value),2)}}</span> ({{__('Purchase Price')}})</span></p>
                  </div>

             @else
                  <div class="form-group">
                    <label for="regular_price">{{ __('Regular Price') }}</label>
                    <input type="number" id="regularPrice" class="form-control" name="regular_price" placeholder="{{ __('Regular Price') }}" value="{{$data->access_status==1 ? 0 :round($data->regular_price-$category->regular_buyer_fee,2)}}" step="0.01" min="0">
                    <p class="text-center mb-0">+</p>
                    <p class="text-center"><span>$ {{ $category->regular_buyer_fee}} {{__('(Buyer Fee)')}}</span> = <span>$ <span id="totalRegularPrice">{{ round(($data->regular_price-$category->regular_buyer_fee) +$category->regular_buyer_fee,2)}}</span> ({{__('Purchase Price')}})</span></p>
                </div>
                <div class="form-group">
                    <label for="extended_price">{{ __('Extended Price *') }}</label>
                    <input type="number" id="extendedPrice" class="form-control" name="extended_price" placeholder="{{ __('Extended Price *') }}" value="{{$data->access_status==1 ? 0 :round($data->extended_price-$category->extended_buyer_fee,2)}}" step="0.01" min="0">
                    <p class="text-center mb-0">+</p>
                    <p class="text-center"><span>$ {{ $category->extended_buyer_fee }} {{__('(Buyer Fee)')}}</span> = <span>$ <span id="totalExtendedPrice">{{ round($category->extended_buyer_fee+($data->extended_price-$category->extended_buyer_fee),2)}}</span> ({{__('Purchase Price')}})</span></p>
                  </div>
              ...
              <div class="lit98_jhy">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="demo_link" name="demo_link"  placeholder="{{ __('Demo Link') }}" value="{{$data->demo_link}}" required>
                  </div>

                  <div class="form-group">
                    <label for="tags">{{ __('Tags') }}</label>
                    <textarea class="form-control"  id="tags" name="tags" required rows="3" placeholder="{{__('Tags')}}">{{$data->tags}}</textarea>
                  </div>
              </div>
              <br>
                        ...
                        <div class="d-flex">
                            <div class="flex-grow-1 pr-3">
                                <div class="form-group mb-1">
                                    <input type="text" class="input-field form-control" name="outcomes[]" placeholder="{{ __('Provide Outcomes') }}" value="{{ $ot }}" required>
                                </div>
                            </div>
                            <div class="">
                              @if($loop->first)
