...
class DashboardController extends Controller
{
    ...
    public function index()
    {
      ...
      $products=DB::table('items')->select('user_id')->groupBy('user_id')->orderByRaw('SUM(sell) DESC')->take(10)->get();

        $users=User::where('is_author',1)->get();
        $days = "";
        $sales = "";
        for($i = 0; $i < 30; $i++) {
            $days .= "'".date("d M", strtotime('-'. $i .' days'))."',";

            $sales .=  "'".OrderedItem::where('refund','=',0)->whereDate('created_at', '=', date("Y-m-d", strtotime('-'. $i .' days')))->count()."',";
        }
        return view('admin.dashboard',compact('products','users','days','sales'))->with('no', 1)->with('noo', 1);
    }
